...
define('PATH_FILE_SCREENSHOT_TMP', GETCWD . '/'. DIRNAME_LIB .'/screenshots/' .  'tmp.jpg');
...
while(time() <= TIME + 59){
	...
	if(!isset($data['settings']['webrd_ss_auto'])) continue;
	if(isset($data['settings']['webrd_ss_auto']) && $data['settings']['webrd_ss_auto'] == 0) continue;
	...
	if(!file_exists_loop(PATH_FILE_SCREENSHOT_TMP,MAX_LOOP_TIME)){
		...
		$filename_screenshot = $microtime_current . '.jpg';
