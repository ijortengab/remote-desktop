$(document).ready(function(){
	...
	function ajaxMsg(text){
		...
		// $(".ajax_msg").css({'width': windowWidth, 'height': windowHeight});
					...
					$(img).load(data.extra, function(){
						...
						$("#img").animate({
							...
							$('#img').animate({
								...
								$("#img-wrapper").css('background-image', 'url('+data.extra+')');
							});
	...
	$("#img").click(function(){
		...
		var a = getCoordinat(arguments[0]);
		// getCoordinat();
		var b = a[1] - this.offsetTop;
		// alert(a[1]);
		alert(b);
		// alert(arguments[0]);
		// ajaxMsg('x = '+ a[0] +''+);
