...
$(document).ready(function(){
	...
	$("#img").mousemove(function(e){		
		...
		// a+= 'typeof($runScroll): ' + typeof( $runScroll ) + ';';
		

		if(CoordinatX > $_startMoveR && $_lengthR != entireWidth) {
			// Simpan temporary value yang akan menjadi nilai perbandingan
			if($('#temporaryX').val().length == 0) $('#temporaryX').val($_startMoveR);


			if(CoordinatX > $('#temporaryX').val()) { // Jika cursor semakin ke kanan, maka geser scroll ke kanan dan ubah cursor menjadi pointer

				// functions.startCarousel(CoordinatX); // jalankan scroll ke kanan
				...
				// functions.stopCarousel(CoordinatX); // stop scroll
		...
		if(CoordinatX < $_startMoveR && $('#temporaryX').val().length != 0){
			$('#temporaryX').val('');
			// functions.stopCarousel(CoordinatX);
		}

		if($_lengthR == entireWidth){
			//beri perintah stop scroll kanan.
			// functions.stopCarousel(CoordinatX);
		}
		a+= 'scrollerInterval: ' + scrollerInterval + ';';
		ajaxMsg(a);
		//nilai temporary kadang masih
	});
	
	functions.startCarousel = function(CoordinatX) {		
	...
	functions.stopCarousel = function(CoordinatX) {
		$('body').css({cursor:'help'}); // set cursor
		// alert( typeOf(scrollerInterval));
		// if (!$_scrollLeft) {
			// alert('ada');
			scrollerInterval = 0;			
		// }
